...
DoGSEA_Viz <-function(DoGSEA=TRUE, InputData, GeneSet, Save=TRUE, FileName, VolcanoPlots=TRUE, VolcanoPlotColour="Standard", LollipopGraphs=TRUE, Lollipop_p.adj =0.25, Lollipop_NES = 0.5){
  if(DoGSEA==TRUE){
    #Load libraries
    library(fgsea)
  ...
  if(LollipopGraphs==TRUE){
    ...
    GSEA_Select <- gsea_result%>%
      subset(padj <= paste(Lollipop_p.adj) & (NES>= Lollipop_NES | NES <= -Lollipop_NES))
   ...
   scale_size(name="NES",range = c(2,16))+
    ...
    dir.create("OutputFigures")
    dir.create("OutputFigures/GSEA_LollipopGraphs") 
    ggsave(paste("OutputFigures/GSEA_LollipopGraphs/LollipopGraph_GSEA_padj",Lollipop_p.adj,"NES",Lollipop_NES, ".pdf"), plot=Dotplot1, width=10, height=10)
    plot(Dotplot1)  
  }
  else{
